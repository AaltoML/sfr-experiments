...
class SFR(nn.Module):
    ...
    @torch.no_grad()
    def predict_f(
        self, x, full_cov: Optional[bool] = False, device: str = "cpu"
    ) -> Tuple[FuncMean, FuncVar]:
        Kxx = self.kernel(x, x, full_cov=full_cov)
    ...
    def optimize_prior_precision(
        ...
        def nlpd_fn(prior_prec: float):
            # This changes the state of the class (recomputes beta_u/alpha_u)
            self.prior_precision = prior_prec
            nll = self.nlpd(
                data_loader=val_loader, pred_type=pred_type, n_samples=n_samples
            )
            logger.info(f"nll: {nll} with prior_prec: {prior_prec}")
            return nll

        # Cache things before tuning
        beta_u_before = copy.deepcopy(self.beta_u)
        Lb_before = copy.deepcopy(self.Lb)
        alpha_u_before = copy.deepcopy(self.alpha_u)
        prior_prec_before = copy.copy(self.prior_precision)
        ...
        if method == "grid":
            ...
            for prior_prec in interval:
                ...
                self.prior_precision = prior_prec
                nll = nlpd_fn(prior_prec=prior_prec)
            ...
            def nlpd_objective(params):
                nll = nlpd_fn(prior_prec=params["prior_prec"])
        ...
        if nll_before < best_nll:
            ...
            # Use this resetting to avoid numerical issues from BO
            self.beta_u = beta_u_before
            self.Lb = Lb_before
            self.alpha_u = alpha_u_before
            self._prior_precision = best_prior_prec
        else:
    ...
    def nlpd(
        self, data_loader: DataLoader, pred_type: str = "gp", n_samples: int = 100
    ):
        try:
            if isinstance(self.likelihood, CategoricalLh) or isinstance(
                ...
                for idx, (x, y) in enumerate(data_loader):
                    p = self(
                        x=x.to(self.device), pred_type=pred_type, num_samples=n_samples
                    )[0]
                    py.append(p)
    ...
    def prior_precision(self, prior_precision):
        logger.info(
            f"Changing prior precision from {self._prior_precision} to {prior_precision}"
        )
        old_prior_precision = copy.copy(self._prior_precision)
        self._prior_precision = copy.copy(prior_precision)
        self.prior.prior_precision = copy.copy(prior_precision)
    ...
    def to_device(self, device):
        ...
        self.beta_u = self.beta_u.to(device)
        self.y_tilde_u = self.y_tilde_u.to(device)
        self.Kzz = self.Kzz.to(device)
        self.Iz = self.Iz.to(device)
