...
class MPPIAgent(Agent):
    ...
    def select_action(
        ...
        for i in range(self.num_mppi_iterations):
            ...
            value = self._estimate_value(
                state, actions, eval_mode=eval_mode
            ).nan_to_num_(0)
    ...
    @torch.no_grad()
    def _estimate_value(
        self, start_state: State, actions: ActionTrajectory, eval_mode: bool = False
    ):
        return self._estimate_value_fn(
            start_state=start_state, actions=actions, eval_mode=eval_mode
        )
